...
const Database = async (dbname) => {
  ...
  const obj = {
    ...
    saveGameScore: async (name, score) => {
      ...
      try{
        let db = getDB();
        let game = await db.get('games');
        game.games.push({name,score});
        await db.put(game);
        await db.close();
        return {status:'success'};
        }catch(error){
          return  {status:'error', message:' Failed to save game score', error: error.message};
        }
      },
    ...
    top10WordScores: async () => {
      ...
      try {
        let db = getDB();
        let wordDB = await db.get('words');
        let arr = wordDB.words;
        ...
        return {status:'error',message:'Failed to retrieve word scores', error: error.message};
      }
    },
    ...
    top10GameScores: async () => {
      ...
      try {
      let db = getDB();
      let gameDB = await db.get('games');
      let arr = gameDB.games;
      let result = arr.sort((a,b) => b.score - a.score);
      result = result.slice(0,10);
      console.log(result);
      await db.close();
      return {status:'success',data: result};
        ...
        return {status:'error',message:'Failed to retrieve game scores', error: error.message};
      }
    },
  };
